    ...
    return (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 w-full px-4 sm:w-auto">
            <nav className="flex items-center justify-center gap-2 px-2 py-2 bg-white/80 backdrop-blur-lg border border-white/20 shadow-2xl rounded-full mx-auto max-w-fit">
                ...
                <Link
                    href="/demo/orders"
                    className={`flex flex-col items-center justify-center min-w-[64px] px-4 h-14 rounded-full transition-all duration-300 ${isActive('/demo/orders')
                        ? 'bg-[#008069] text-white shadow-lg translate-y-[-4px]'
                        : 'text-gray-500 hover:text-[#111813] hover:bg-gray-100'
                        }`}
                >
                    <ShoppingBag className={`w-5 h-5 ${isActive('/demo/orders') ? 'fill-current' : ''}`} />
                ...
                <Link
                    href="/demo"
                    className={`flex flex-col items-center justify-center min-w-[64px] px-4 h-14 rounded-full transition-all duration-300 ${isActive('/demo')
                        ? 'bg-[#008069] text-white shadow-lg translate-y-[-4px]'
                        : 'text-gray-500 hover:text-[#111813] hover:bg-gray-100'
                        }`}
                >
                    <Home className={`w-5 h-5 ${isActive('/demo') ? 'fill-current' : ''}`} />
                    <span className="text-[10px] font-bold mt-1 text-center leading-3 whitespace-nowrap">
                ...
                >
                    <Users className={`w-5 h-5 ${isActive('/demo/customers') ? 'fill-current' : ''}`} />
                    <span className="text-[10px] font-bold mt-1 text-center leading-3 whitespace-nowrap">
