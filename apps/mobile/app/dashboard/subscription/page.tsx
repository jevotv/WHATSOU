...
export default function SubscriptionPage() {
    ...
    // Local state for transactions since it's not in context
    const [transactions, setTransactions] = useState<any[]>([]);
    const [loadingHistory, setLoadingHistory] = useState(true);

    // Derived loading state
    const loading = !subscription; // Initial loading if subscription is null
    ...
    async function loadHistory() {
        ...
        try {
            const data = await api.get<{ transactions: any[] }>('/api/dashboard/subscription/history');
    ...
    async function handlePayment() {
        ...
        if (paymentMethod === 'instapay' || paymentMethod === 'vodafone') {
            try {
                const result = await api.post<any>('/api/dashboard/subscription/manual-pay', {
        ...
        // Card payment
        try {
            const result = await api.post<any>('/api/dashboard/subscription/pay', {
                paymentMethod: 'card'
            });
        ...
        <div className="p-4 max-w-2xl mx-auto space-y-6" dir={direction}>
            ...
            <div className={`rounded-2xl ${config.bgColor} p-6 shadow-sm`}>
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => { refresh(); loadHistory(); }} className="p-2 hover:bg-white/50 rounded-full transition">
